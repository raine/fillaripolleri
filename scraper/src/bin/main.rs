...
fn handle_feed_topic(
    ...
    let snapshot_created = create_topic_snapshot(conn, feed_topic)?;
    if snapshot_created {
        // New topic snapshot means topic is new or subject and/or message has changed, and
        // should parse the topic data into an Item.
        let topic_with_snapshots =
            get_topic_with_snapshots(conn, &feed_topic.guid)?.expect("query should return a topic");

        // TODO: Create an item from topic_with_snapshots
